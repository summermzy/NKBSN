...
class Net(nn.Module):
    def __init__(self, __C, pretrained_emb, token_size, answer_size, ix_to_token):
        ...
        self.qus_feat_lstm = nn.LSTM(
            input_size=__C.ELMO_FEAT_SIZE,
            hidden_size=__C.HIDDEN_SIZE,
            num_layers=1,
            batch_first=True
        )
    ...
    def forward(self, img_feat, ques_ix):
        ...
        if self.USE_GLOVE:
            ...
            # elmo word embedding
            ques_content_iter = [[self.ix_to_token[ix] for ix in single_ques_ix.cpu().numpy()] for single_ques_ix in ques_ix]
            character_ids = batch_to_ids(ques_content_iter)
            character_ids = character_ids.to(ques_ix.device)
            elmo_embedding = self.elmo(character_ids)['elmo_representations'][0]
            lang_feat, _ = self.qus_feat_lstm(elmo_embedding)
